...
import { 
  ...
  Smartphone, Shield, Eye, Search, AlertTriangle 
} from "lucide-react";
...
const AdminReferidos = () => {
  ...
  const [tab, setTab] = useState<"pendientes" | "aprobados" | "rechazados" | "puntos">("pendientes");
  ...
  const otorgarPuntos = async () => {
    if (!selectedUserId || !puntos || parseInt(puntos) <= 0) {
      toast.error("Selecciona usuario y puntos válidos");
      return;
    }
    setSubmitting(true);
    const { data: { user } } = await supabase.auth.getUser();
  ...
  const renderReferidoCard = (ref: ReferidoAdmin, showActions: boolean) => (
    <div key={ref.id} className="border border-border rounded-lg p-4 space-y-3 bg-card">
      <div className="flex items-start justify-between gap-3">
        ...
        <Badge 
          variant={ref.aprobado ? "default" : ref.rechazado ? "destructive" : "outline"} 
      ...
      <div className="bg-muted/50 rounded-md p-3 space-y-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
          <p className="text-xs text-muted-foreground">
            <span className="font-medium">Tipo:</span> {getDeviceIcon(ref.user_agent)}
          </p>
          <p className="text-xs text-muted-foreground">
            <span className="font-medium">IP:</span> {ref.ip_aproximada || "No registrada"}
          </p>
        </div>
        {ref.user_agent && (
          <p className="text-[10px] text-muted-foreground/70 break-all font-mono leading-tight">
            {ref.user_agent.substring(0, 120)}...
          </p>
        )}
      ...
      {showActions && (
        <div className="space-y-2">
          <Textarea
            ...
            value={notaAdmin[ref.id] || ""}
            onChange={(e) => setNotaAdmin(prev => ({ ...prev, [ref.id]: e.target.value }))}
            className="text-xs min-h-[60px]"
          />
          <div className="flex gap-2">
            <Button size="sm" onClick={() => aprobarReferido(ref)} className="flex-1">
              <CheckCircle className="h-3.5 w-3.5 mr-1" /> Aprobar
            </Button>
            <Button size="sm" variant="destructive" onClick={() => rechazarReferido(ref)} className="flex-1">
      ...
      {ref.fecha_revision && (
        <p className="text-[10px] text-muted-foreground">
          Revisado: {new Date(ref.fecha_revision).toLocaleDateString("es-DO")}
        </p>
      )}
    </div>
  );
        ...
        {loading ? (
          ...
          <>
                ...
                {pendientes.length === 0 ? (
                  <Card className="border">
                    <CardContent className="py-12 text-center">
                      ...
                      <p className="text-sm text-muted-foreground">No hay referidos pendientes de aprobación</p>
                    </CardContent>
                  </Card>
                ) : (
            ...
            {tab === "aprobados" && (
              <div className="space-y-3">
                {aprobados.length === 0 ? (
                  <p className="text-center py-12 text-sm text-muted-foreground">No hay referidos aprobados</p>
                ) : (
            ...
            {tab === "rechazados" && (
              <div className="space-y-3">
                {rechazados.length === 0 ? (
                  <p className="text-center py-12 text-sm text-muted-foreground">No hay referidos rechazados</p>
                ) : (
              ...
              <Card className="border">
                ...
                <CardContent className="space-y-4">
                      ...
                      {searchUser && filteredUsers.length > 0 && (
                        <div className="border rounded-md max-h-40 overflow-y-auto">
                          {filteredUsers.slice(0, 8).map(u => (
                            <button
                              key={u.user_id}
                              onClick={() => { setSelectedUserId(u.user_id); setSearchUser(u.nombre_completo || u.correo); }}
                  ...
                  <div className="border-t border-border pt-4 mt-4">
                    <h3 className="text-sm font-semibold mb-3">Ranking de Puntos</h3>
